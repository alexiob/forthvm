defmodule ForthVM.Words.Flow do
  ...
  def _do(tokens, [count, end_count | data_stack], return_stack, dictionary, meta)  do
    Core.next(tokens, data_stack, [count, end_count, %{do: tokens} | return_stack], dictionary, meta)
  end

  @doc"""
  i: ( -- count ) copy the top of a LOOP's return stack to the data stack
  """
  def i(tokens, data_stack, [count, _end_count, %{do: _do_tokens} | _] = return_stack, dictionary, meta)  do
  ...
  """
  def copy_r_loop_to_d(tokens, data_stack, [_count, _end_count, %{do: _do_tokens}, data | _] = return_stack, dictionary, meta)  do
    Core.next(tokens, [data | data_stack], return_stack, dictionary, meta)
  end
  ...
  """
  def loop(tokens, data_stack, [count, end_count, %{do: do_tokens} | return_stack], dictionary, meta)  do
    count = count + 1

    case count < end_count do
      true -> Core.next(do_tokens, data_stack, [count, end_count, %{do: do_tokens} | return_stack], dictionary, meta)
      false -> Core.next(tokens, data_stack, return_stack, dictionary, meta)
    end
  end
  ...
  """
  def plus_loop(tokens, [inc | data_stack], [count, end_count, %{do: do_tokens} | return_stack], dictionary, meta)  do
    count = count + inc
  ...
  """
  def until(tokens, [condition | data_stack], [%{begin: until_tokens} | return_stack], dictionary, meta)  do
    case is_falsely(condition) do
      true -> Core.next(until_tokens, data_stack, [%{begin: until_tokens} | return_stack], dictionary, meta)
      false -> Core.next(tokens, data_stack, return_stack, dictionary, meta)
    end
  end
