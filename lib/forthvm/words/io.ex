defmodule ForthVM.Words.IO do
  ...
  """
  def set_io_device(tokens, [device_name | data_stack], return_stack, dictionary, %{io: %{device: device, devices: devices}} = meta) do
    device = Map.get(devices, device_name, device)

    Core.next(tokens, data_stack, return_stack, dictionary, %{meta | io: Map.put(meta.io, :device, device)})
  end
  ...
  """
  def dump_data_stack(tokens, data_stack, return_stack, dictionary, %{io: %{device: device}} = meta) do
    data_stack
    |> Enum.reverse()
