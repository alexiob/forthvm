defmodule ForthVM.CoreWordsInterpreterTest do
  ...
  test "abort? from inside function, truthly" do
    assert capture_io(fn ->
             assert {:exit, _, nil} =
                      core_run(~s[: trigger abort? "see yah!" ; true trigger 42 dup dup])
           end) == "see yah!\n"
  end

  test "abort? from inside function, falsely" do
    assert {:exit, _, [42, 42, 42]} =
             core_run(~s[: trigger abort? "see yah!" ; false trigger 42 dup dup])
  end
  ...
  test "setting an already defined constant should error" do
    assert {:error, _, "can not set already defined constant 'test' with new value '1'"} =
             core_run("42 constant test 1 constant test")
  end

  test "include a file" do
    assert capture_io(fn ->
             assert {:exit, _, _} =
                      core_run(~w[include test/fixtures/hello-world.forth hello-world])
  ...
  test "include an unknown file sould error" do
    assert {:error, _, "can not include 'unknown.forth' because ':enoent'"} =
             core_run(~w[include unknown.forth hello-world])
  end
end
